...
class ServerGame extends ServerEngine {
      ...
      for (let i = 0, j = pairs.length; i !== j; ++i) {
        ...
        if(pair.bodyA === this.leftSensor || pair.bodyB === this.leftSensor) {
          ...
          for (let i = 0; i < this.players.length; i++) {
            if (this.players[i].ship.position.x < 900) {
              Matter.Body.setVelocity(this.players[i].ship, { x: 80, y: -(this.ball.position.y - this.players[i].ship.position.y) / 6 });
            }
          }
          ...
          for (let i = 0; i < this.players.length; i++) {
            if (this.players[i].ship.position.x > 2900) {
              Matter.Body.setVelocity(this.players[i].ship, { x: -80, y: -(this.ball.position.y - this.players[i].ship.position.y) / 6 });
            }
          }
          Matter.Body.setPosition(this.ball, { x: -9999, y: 900 })
  ...
  addNewPlayer(id) {
    this.roster.set(id, new ServerPlayer(id, this.world));
  ...
  reset() {
    for (let i = 0; i < this.players.length; i++) {
      if (i === 0) {
        Matter.Body.setVelocity(this.players[i].ship, { x: 0, y: 0 });
        Matter.Body.setPosition(this.players[i].ship, { x: 450, y: 900 });
      }
      if (i === 1) {
        Matter.Body.setVelocity(this.players[i].ship, { x: 0, y: 0 });
        Matter.Body.setPosition(this.players[i].ship, { x: 1150, y: 900 });
      }
    }
