  ...
  init() {
    ...
    Matter.Events.on(this.engine, "collisionStart", (event) => {
      ...
      for (let i = 0, j = pairs.length; i !== j; ++i) {
        ...
        if ((pair.bodyA === this.leftSensor || pair.bodyB === this.leftSensor) && (pair.bodyA === this.ball || pair.bodyB === this.ball) ) {
          this.score.RIGHT += 1;
          // // this.timeout = 65;
          // Matter.Body.setPosition(this.leftExplosion, { x: -1000, y: 900})
          // Matter.Body.applyForce(this.leftExplosion, this.leftExplosion.position, { x: 10, y: 0})
          // Matter.Body.applyForce(this.ball, this.ball.position, { x: 10, y: 0})
          Matter.Body.setVelocity(this.ball, { x: 0, y: 0 });
          Matter.Body.setAngularVelocity(this.ball, 0);
          const players = this.roster.values();
          for (let i = 0; i < players.length; i++) {
            if (players[i].ship.position.x < 900) {
              Matter.Body.setVelocity(players[i].ship, { x: 80, y: -(this.ball.position.y - players[i].ship.position.y) / 4 });
            }
          }
        ...
        } else if ((pair.bodyA === this.rightSensor || pair.bodyB === this.rightSensor)  && (pair.bodyA === this.ball || pair.bodyB === this.ball) ) {
          this.score.LEFT += 1;
          // this.timeout = 65;
          Matter.Body.setVelocity(this.ball, { x: 0, y: 0 });
          Matter.Body.setAngularVelocity(this.ball, 0);
